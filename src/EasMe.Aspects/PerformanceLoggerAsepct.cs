...
/// <summary>
///    Performance Logger Aspect for logging long running actions, with a default threshold of 1000 ms.
///    Desired threshold can be set in constructor.
///    It uses SeriLog for logging.
///    If you have not configured serilog this will not work.
/// </summary>
      ...
      [Argument(Source.ReturnType)] Type returnType)
   {
      ...
      try {
         ...
         if (_logThresholdMilliseconds < sw.ElapsedMilliseconds)
            Log.Warning(
               "{className}.{methodName}({argList}) Action too long to execute, executed in {ElapsedMilliseconds} ms",
               className, methodName, argList, sw.ElapsedMilliseconds);
      }
   }
}